...
## TG - Pagination with absolute URLs

**Spec requirement:** Link headers with absolute URLs must be used directly without modification.
  ...
  onRequest: (req) => {
    ...
    IF request_count == 1:
      req.respond_with(200,
        body: [{ "id": "item1" }],
        headers: {
          "Link": "<https://rest.ably.io/channels/test/messages?cursor=abc>; rel=\"next\""
        }
      )
    ELSE:
      req.respond_with(200, body: [{ "id": "item2" }])
...
### Assertions
```pseudo
ASSERT captured_requests[1].url.scheme == "https"
ASSERT captured_requests[1].url.host == "rest.ably.io"
ASSERT captured_requests[1].url.query_params["cursor"] == "abc"
```
