...
- `(RTLM17)` The initial value from `ObjectOperation.map` can be merged into this `LiveMap` in the following way:
  - `(RTLM17a)` For each key-`ObjectsMapEntry` pair in `ObjectOperation.map.entries`:
    ...
    - `(RTLM17a2)` If `ObjectsMapEntry.tombstone` is `true`, apply the `MAP_REMOVE` operation to the current key as described in [RTLM8](#RTLM8), passing in the current key as `ObjectsMapOp`, and `ObjectsMapEntry.timeserial` as `serial`. Store the returned `LiveMapUpdate` object for use in [RTLM17c](#RTLM17c)
