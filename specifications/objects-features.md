...
- `(RTPO20)` `PathObject#batch` function:
  ...
  - `(RTPO20f)` After `fn` returns, flushes the `RootBatchContext` ([RTBC16d](#RTBC16d)) to publish all queued operations atomically
  - `(RTPO20g)` The `RootBatchContext` is closed after flush completes, regardless of success or failure
...
- `(RTINS17)` `Instance#batch` function:
  ...
  - `(RTINS17f)` After `fn` returns, flushes the `RootBatchContext` ([RTBC16d](#RTBC16d)) to publish all queued operations atomically
  - `(RTINS17g)` The `RootBatchContext` is closed after flush completes, regardless of success or failure
...
Types and their properties/methods are public and exposed to users by default. An `internal` label may be used to indicate that a type or its property/method must not be exposed to users and is intended for internal SDK use only.
    ...
    class PathObject: // RTPO*
      ...
      batch(((BatchContext) ->) fn) => io // RTPO20
    ...
    class Instance: // RTINS*
      ...
      batch(((BatchContext) ->) fn) => io // RTINS17
