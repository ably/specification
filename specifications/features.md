...
- `(TO3)` The attributes of `ClientOptions` consist of:
  ...
  - `(TO3g)` `queueMessages` boolean - defaults to true. If false, suppresses the default queueing of `MESSAGE` or `PRESENCE` `ProtocolMessage`s when connection is either in the `CONNECTING` or `DISCONNECTED` states. Otherwise those messages should be failed immediately if not in the `CONNECTED` state.
