...
- `(CSV2)` **Service Version**: This document describes requirements for client libraries that are compatible with Ably endpoints at a specific service version (also referred to as the 'wire protocol' version).
  - `(CSV2a)` The service version is an Integer value (from service version `2` onwards - it was a Decimal prior to that, for example `1.2`).
  - `(CSV2b)` This document is compatible with service version `{{ SERVICE_VERSION }}`.
  - `(CSV2c)` A client library must identify to Ably the service version it uses in all requests and connections, per [`RSC7a`](#RSC7a) and [`RTN2f`](#RTN2f).
  - `(CSV2d)` It is expected, very likely required, that client libraries which implement most or all of this specification at the version defined in this document ([`CSV1b`](CSV1b)) will need to identify with Ably at the service version defined in this document ([`CSV2b`](#CSV2b)). There may, from time to time, be edge cases where this is not the case but these must be considered very carefully (for example a client library using an older service version until fully implemented).
  - `(CSV2e)` Client libraries that need to send a Decimal service version must send the exact string specified in [`CSV2b`](#CSV2b) (previously `G4`). Therefore, it is recommended that client library implementations treat the version opaquely, as a string, not a float.
...
- `(RTN16)` `Connection` recovery:
  ...
  - `(RTN16b)` This clause has been removed as of the 2.0 spec and replaced with `RTN16g` and `RTN16m`.
  - `(RTN16c)` This clause has been replaced by RTN16g2 as of the 2.0 spec.
...
- `(RTL15)` `RealtimeChannel#properties` attribute is a `ChannelProperties` object representing properties of the channel state. `properties` is a publicly accessible member of the channel, but it is an experimental and unstable API. It has the following attributes:
  ...
  - `(RTL15b)` `channelSerial` is updated whenever a `ProtocolMessage` with either `MESSAGE`, `PRESENCE`, or `ATTACHED` actions is received on a channel, and is set to the `TR4c` `channelSerial` of that `ProtocolMessage`, if and only if that field (`ProtocolMessage.channelSerial`) is populated.
...
- `(RTP17)` The RealtimePresence object should also keep a second `PresenceMap` containing only members that match the current `connectionId`. Any incoming presence message that satisfies `RTP17b` should be applied to this object in the same way as for the normal `PresenceMap`. This object should be private and is used to maintain a list of members that need to be automatically re-entered by the `RealtimePresence` object when required to by `RTP17f`.
  ...
  - `(RTP17c)` This clause and its subclauses have been replaced by `RTP17f` as of the 2.0 spec.
  - `(RTP17d)` This clause has been replaced by `RTP17g` as of the 2.0 spec.
...
- `(CP2)` The attributes of `ChannelProperties` consist of:
  ...
  - `(CP2b)` `channelSerial` string - contains the last `channelSerial` received in any `MESSAGE`, `PRESENCE`, or `ATTACHED` `ProtocolMesage` on the channel, see [RTL15b](#RTL15b)
