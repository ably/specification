...
- `(RSL6)` Message decoding
  - `(RSL6a)` All messages received will be decoded automatically based on the `encoding` field and the payloads will be converted into the format they were originally sent using i.e. binary, string, or JSON
    ...
    - `(RSL6a3)` A set of test should exist that ensure that the client library can successfully encode and decode binary encoded protocol-messages. The test should use the [set of predefined interoperability msgpack protocol-message fixtures](https://github.com/ably/ably-common/blob/main/test-resources/msgpack_test_fixtures.json). The msgpack field in this fixture represents the base64 encoding of the entire protocol-message, but the "data" field represents the first element of the messages field of the protocol-message.
