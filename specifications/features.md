...
- `(RTP16)` Connection state conditions:
  ...
  - `(RTP16b)` If the channel is `ATTACHING` or `INITIALIZED`, then if `ClientOptions.queueMessages` is `true`, the presence messages should be queued at a channel level, to be handled per `RTP16a` once the channel becomes `ATTACHED`, or failed if the channel state becomes `SUSPENDED`, `FAILED`, or `DETACHED` first
