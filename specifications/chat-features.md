...
- `(CHA-RL1)` A room must be explicitly attached via the `ATTACH` operation.
  ...
  - `(CHA-RL1k)` `[Testable]` Following the `CHA-RL1e` transition, the client shall initiate and await the attachment of the realtime channel.
    - `(CHA-RL1k1)` `[Testable]` If the attachment is successful, the room status shall be transitioned to `ATTACHED`, and the `isPostExplicitDetach` and `isFirstAttach` flags set to `false`.
    ...
    - `(CHA-RL1k3)` `[Testable]` If the attachment is unsuccessful (i.e. `channelattach()` throws an error), the attach error will be thrown as the result of the room `attach()` operation.
