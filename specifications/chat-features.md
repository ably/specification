...
- `(CHA-MR2)` There are three types of message reactions, each corresponding to a different annotation aggregation type.\
  ...
  `(CHA-MR2b)` `[Testable]` Reactions of type `Distinct` use the annotation type `reaction:distinct.v1`. Each client can add multiple reaction with different names, but each unique `name` only once (similar to Slack).\
  `(CHA-MR2c)` `[Testable]` Reactions of type `Multiple` use the annotation type `reaction:multiple.v1`. In this type, each client can add any reaction `name` multiple times including duplicates. Reactions can include a `count` field indicating how many times the reaction should be counted towards the total (similar to claps on Medium).
...
- `(CHA-MR9)` The `MessageReactionRawEvent` interface provides information about individual message reaction events.\
  ...
  \* `(CHA-MR9c3)` `[Testable]` A `name` field containing the name of the reaction (e.g. emoji string).\
