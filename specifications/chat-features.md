...
- `(CHA-M9)` A client must be able to delete a message in a room.
  ...
  - `(CHA-M9d)` `[Testable]` A client must be able to delete a message without requiring the full `Message` type. At minimum, it must be able to provide a `serial` string and an (optional) operation information.
    - `(CHA-M9d1) The exact idiom for the method signature is left to the best practice for the language.
      ** `(CHA-M9e)@ `[Testable]` If the `serial` passed to this method is invalid: `undefined, null, empty string`, an error with code 40000 must be thrown.
- `(CHA-M4)` Messages can be received via a subscription in realtime.
  ...
  - `(CHA-M4m)` `[Testable]` The `action` field of the realtime message determines the type of chat message event that is emitted, based on the following rules.
    ...
    - `(CHA-M4m5)` `[Testable]` When parsing an incoming message with action `MessageDelete`, then the messages `text` property must be set to an empty string.
    - `(CHA-M4m6)` `[Testable]` When parsing an incoming message with action `MessageDelete`, then the messages `metadata` property must be set to an empty object.
    - `(CHA-M4m7)` `[Testable]` When parsing an incoming message with action `MessageDelete`, then the messages `headers` property must be set to an empty object.
  - `(CHA-M4k)` `[Testable]` Incoming realtime events that are malformed (unknown fields should be ignored) shall not be emitted to subscribers.
    - `(CHA-M4k1)` `[Testable]` If the message does not contain a `data` field, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k2)` `[Testable]` If the message does not contain a `clientId` field, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k3)` `[Testable]` If the message does not contain an `extras` field, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k4)` `[Testable]` If the message action is not `MessageDelete` and the message `data` field does not contain a `text` property or the `text` property is undefined, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k5)` `[Testable]` If the message action is not `MessageDelete` and the message `extras` field does not contain a `headers` property, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k6)` `[Testable]` If the message action is not `MessageDelete` and the message `data` field does not contain a `metadata` property, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k7)` `[Testable]` If the message does not contain a `serial` field, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k8)` `[Testable]` If the message does not contain a `version` field, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k9)` `[Testable]` If the message does not contain a `createdAt` field, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k10)` `[Testable]` If the message does not contain a `timestamp` field, an `ErrorInfo` with code `50000` must be thrown.
    - `(CHA-M4k11)` `[Testable]` If the message contains an `action` field with a value that is not one of the supported `ChatMessageAction` values (`MessageCreate`, `MessageUpdate`, `MessageDelete`), an `ErrorInfo` with code `50000` must be thrown.
...
- `(CHA-MR4)` `[Testable]` Users should be able to send a reaction to a message via the \`send\` method of the \`MessagesReactions\` object (`room.messages.reactions.send`).
  - `(CHA-MR4a)` `[Testable]` The \`send\` method accepts a message (or message serial) as the first parameter to identify which message to react to.
    - `(CHA-MR4a1)` `[Testable]` If the `serial` passed to this method is invalid: `undefined, null, empty string`, an error with code 40000 must be thrown.
...
- `(CHA-MR11)` `[Testable]` Users should be able to delete a reaction from a message via the \`delete\` method of the \`MessagesReactions\` object (`room.messages.reactions.delete`).
  - `(CHA-MR11a)` `[Testable]` The \`delete\` method accepts a message (or message serial) as the first parameter to identify which message to delete the reaction from.
    - `(CHA-MR11a1)` `[Testable]` If the `serial` passed to this method is invalid: `undefined, null, empty string`, an error with code 40000 must be thrown.
...
- `(CHA-ER3)` Ephemeral room reactions are sent to Ably via the Realtime connection via a `send` method.
  ...
  - `(CHA-ER3f)` `[Testable]` If the connection state is not `CONNECTED`, the operation shall throw an `ErrorInfo` with code `40000`.
...
- `(CHA-PR7)` Users may subscribe to presence events.
  - `(CHA-PR7d)` `[Testable]` If presence events have been disabled per `CHA-PR9c`, then the client shall throw an error with code `40000`.
  ...
  - `(CHA-PR7b)` `[Testable]` Users may provide a listener and a list of selected [presence events](#chat-structs-presence-event-v2), to subscribe to just those events in a room.
...
- `(CHA-T16)` `[Testable]` Users must be able to retrieve a list of the currently typing client IDs through a `typing.current()` method. The list must be a copy of the up-to-date internal list of typing client IDs.
...
- `(CHA-T4)` Users may indicate that they have started typing using the `keystroke` method.
  ...
  - `(CHA-T4e)` `[Testable]` If the connection state is not `CONNECTED`, the operation shall throw an `ErrorInfo` with code `40000`.
...
- `(CHA-T5)` Users may explicitly indicate that they have stopped typing using `stop` method.
  ...
  - `(CHA-T5f)` `[Testable]` If the connection state is not `CONNECTED`, the operation shall throw an `ErrorInfo` with code `40000`.
