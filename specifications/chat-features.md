...
- `(CHA-RL3)` A room must be explicitly released via the `RELEASE` operation. This operation is not performed directly on a Room object by the client, but is described here.
  ...
  - `(CHA-RL3m)` `[Testable]` When the release operation commences, the room will transition into the `RELEASING` status before attempting to detach and then release the realtime channel.
