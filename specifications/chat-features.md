...
- `(CHA-RL1)` A room must be explicitly attached via the `ATTACH` operation.
  ...
  - `(CHA-RL1i)` This specification point has been removed, since it's not needed as per CHA-RC1.
    - `(CHA-RL1i1)` `[Testable]` This specification point has been removed, since it's not needed as per CHA-RC1.
