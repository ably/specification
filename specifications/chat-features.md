...
- `(CHA-M4)` Messages can be received via a subscription in realtime.
  ...
  - `(CHA-M4k)` `[Testable]` Incoming realtime events that are malformed, or have incomplete data, shall still emit an event, but fields will be replaced with default values.
    - `(CHA-M4k1)` `[Testable]` Text fields (e.g. serial, string, version, clientId) shall be empty strings.
    - `(CHA-M4k2)` `[Testable]` Object fields (e.g. headers, metadata) shall be empty maps/objects.
    - `(CHA-M4k3)` `[Testable]` The `timestamp` field shall have the current time.
    - `(CHA-M4k4)` `[Testable]` The `createdAt` field shall be 0 (the unix epoch).
...
- `(CHA-M13)` A single message should be queryable from the REST API.
  - `(CHA-M13a)` `[Testable]` A method must be exposed that accepts a message serial and returns a single `Message` object. It shall call the "REST API"#rest-fetching-single-message and return the message if found.
  - `(CHA-M13b)` `[Testable]` If the REST API returns a 404 error, then the method must return `null` or the language's idiomatic equivalent for "not found".
  - `(CHA-M13c)` `[Testable]` If the REST API returns any other error, then the method must throw its `ErrorInfo` representation.
