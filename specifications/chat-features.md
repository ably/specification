...
- `(CHA-RL4)` A room must monitor the state of its `contributors` Realtime Channels and act upon any changes.
  - `(CHA-RL4a)` The state monitor must handle `UPDATE` events from each contributors underlying Realtime Channel
    ...
    - `(CHA-RL4a2)` `[Testable]` If the given contributor has not yet successfully managed to attach its Realtime Channel (i.e. a call to `attach()` on the channel, per `CHA-RL1f`, has succeeded), then no action should be taken.
  ...
  - `(CHA-RL4b)` The state monitor must handle other channel state events.
    - `(CHA-RL4b1)` `[Testable]` If a channel lifecycle operation is in progress, and the new channel state is `ATTACHED`, and the `resumed` flag is false, `and` the particular contributor has been attached previously, then a pending discontinuity event will be recorded for the contributor. The error associated with this event shall be the `reason` for the channel state change.
