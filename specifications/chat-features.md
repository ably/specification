...
- `(CHA-O4)` Users can subscribe to realtime occupancy updates.
  ...
  - `(CHA-O4e)` `[Testable]` When a regular occupancy event is received on the channel (a standard PubSub occupancy event per the docs), the SDK will convert it into [occupancy event V2](#chat-structs-occupancy-event-v2) format, with type `OccupancyEventType.Updated`, and broadcast it to subscribers.
