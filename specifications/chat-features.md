...
- `(CHA-RL1)` A room must be explicitly attached via the `ATTACH` operation.
  ...
  - `(CHA-RL1b)` `[Testable]` If the room is in the `RELEASING` status, the operation shall be rejected with an `ErrorInfo` with code `102102`.
  - `(CHA-RL1c)` `[Testable]` If the room is in the `RELEASED` status, the operation shall be rejected with an `ErrorInfo` with code `102103`.
...
- `(CHA-RL2)` A room must be explicitly detached via the `DETACH` operation.
  ...
  - `(CHA-RL2c)` `[Testable]` If the room is in the `RELEASED` status, the operation shall be rejected with an `ErrorInfo` with code `102103`.
  - `(CHA-RL2d)` `[Testable]` If the room is in the `FAILED` status, the operation shall be rejected with an `ErrorInfo` with code `102101`.
