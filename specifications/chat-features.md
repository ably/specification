...
- `(CHA-RL4)` A room must monitor the state of its `contributors` Realtime Channels and act upon any changes.
  ...
  - `(CHA-RL4b)` The state monitor must handle other channel state events.
    ...
    - `(CHA-RL4b4)` `[Testable]` If a channel lifecycle operation is in progress, the channel state is `ATTACHED`, the `resumed` flag is false but the contributor has never previouly reached the state of `ATTACHED`, then no action is taken.
