...
- `(CHA-RL13)` The room lifecycle shall maintain a `isFirstAttach` flag, which represents whether or not the next `ATTACHED` channel state change is the first one received (i.e. the product of a call to `ATTACH`). It defaults to `false`. Subsequent specification items will explain how this flag is manipulated.
- `(CHA-RL14)` The room lifecycle shall maintain a `isPostExplicitDetach` flag, which represents whether or not the current room `DETACHED` status is the result of a user-initiated `detach` operation. It defaults to `false`. Subsequent specification items will explain how this flag is manipulated.
