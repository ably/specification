...
- `(CHA-RL5)` A room must `RETRY` whenever it enters the `SUSPENDED` state.
  - `(CHA-RL5a)` `[Testable]` When entering a retry-loop, the room must first `DETACH` all contributors underlying realtime channels using a `CHA-RL2f` detachment cycle, with the exception of the channel that became `SUSPENDED`.
