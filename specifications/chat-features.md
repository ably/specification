...
- `(CHA-PR9)` Users may configure their presence options via the `RoomOptions` provided at room configuration time.
  ...
  - `(CHA-PR9c)` The option `enablePresenceEvents` controls whether or not the client should receive presence events from the server.
...
- `(CHA-O4)` Users can subscribe to realtime occupancy updates.
  - `(CHA-04e)` `[Testable]` If occupancy events have been disabled per `CHA-O6`, then the client shall throw an error with code `40000`.
...
- `(CHA-O6)` Enabling occupancy events causes extra messages to be received by the client. Therefore, whether to receive these messages shall be user-configurable.
  - `(CHA-O6a)` `[Testable]` If `occupancy.enableOccupancyEvents` is set to `true`, then the client shall set the `options.params.occupancy` channel parameter to `"metrics"` on the underlying realtime channel per `CHA-RC3`.
  - `(CHA-O6b)` `[Testable]` If `occupancy.enableOccupancyEvents` is set to `false`, then the client shall not set the `options.params.occupancy` channel parameter.
  - `(CHA-O6c)` `[Testable]` The default value of `occupancy.enableOccupancyEvents` is `false`.
