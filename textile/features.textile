...
*** @(RTN15g4)@ When the connection state is cleared in this way, the internal @msgSerial@ counter should be reset to zero, just as if the connection were a resume attempt which failed per "RTN15c7":#RTN15c7.
...
*** @(RTN19a2)@ In the case of an @RTN15c6@ successful resume, the @msgSerial@ of the reattempted @ProtocolMessage@s should remain the same as for the original attempt. In the case of an @RTN15c7@ failed resume or when the connection state is cleared per @RTN15g@, the message must be assigned a new @msgSerial@ from the SDK's internal counter.
