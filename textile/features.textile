...
*** @(RSAN1a4)@ The SDK may validate that the total size of the annotation or (if publishing an array) annotations, calculated identially to message size per "TO3l8":#TO3l8, exceeds the @maxMessageSize@, then the client library should reject the publish and indicate an error with code 40009
...
** @(TM2q)@ @summary@ @Dict<string, JsonObject>@ - will always be populated for a message with action `MESSAGE_SUMMARY`, and may be populated for any other type. The keys of the dict are annotation types, and the values are aggregated summaries for that annotation type.
...
** @(TM7b)@ If the SDK chooses to do this, it should support at least the following set of constructors:
