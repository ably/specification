...
* @(OMO1)@ A @MapOp@ describes an operation to be applied to a @Map@ object
* @(OMO2)@ The attributes available in a @MapOp@ are:
...
* @(OMO3)@ The size of the @MapOp@ is calculated as follows:
...
* @(OCO1)@ A @CounterOp@ describes an operation to be applied to a @Counter@ object
* @(OCO2)@ The attributes available in a @CounterOp@ are:
** @(OCO2a)@ @amount@ number - the data value that should be added to the counter
* @(OCO3)@ The size of the @CounterOp@ is calculated as follows:
** @(OCO3a)@ The size is 8 if @amount@ is a number
** @(OCO3b)@ The size is 0 if @amount@ is a @null@ or omitted

h4. ObjectMap

* @(OMP1)@ A @Map@ object represents a map of key-value pairs
* @(OMP2)@ @Map@ @Semantics@ enum has the following values in order from zero: @LWW@
* @(OMP3)@ The attributes available in a @Map@ are:
** @(OMP3a)@ @semantics@ @Semantics@ enum - the conflict-resolution semantics used by the map object
** @(OMP3b)@ @entries@ @Dict<String, MapEntry>@ - the map entries, indexed by key
* @(OMP4)@ The size of the @Map@ is calculated as follows:
...
*** @(OMP4a2)@ Includes the size of the @MapEntry@ object for the map entry, calculated per "OME3":#OME3
** @(OMP4b)@ The size of a @null@ or omitted property is zero

h4. ObjectCounter

* @(OCN1)@ A @Counter@ object represents an incrementable and decrementable value
* @(OCN2)@ The attributes available in a @Counter@ are:
** @(OCN2a)@ @count@ number - the value of the counter
* @(OCN3)@ The size of the @Counter@ is calculated as follows:
** @(OCN3a)@ The size is 8 if @count@ is a number
** @(OCN3b)@ The size is 0 if @count@ is a @null@ or omitted

h4. ObjectMapEntry

* @(OME1)@ A @MapEntry@ represents the value at a given key in a @Map@ object
* @(OME2)@ The attributes available in a @MapEntry@ are:
...
* @(OME3)@ The size of the @MapEntry@ is calculated as follows:
...
enum MapSemantics: // OMP2, internal
  LWW // OMP2
...
class ObjectMapOp // OMO*, internal
  key: String // OMO2a
  data: ObjectData? // OMO2b

class ObjectCounterOp // OCO*, internal
  amount: Number // OCO2a

class ObjectMap // OMP*, internal
  semantics: MapSemantics // OMP3a
  entries: Dict<String, MapEntry>? // OMP3b

class ObjectCounter // OCN*, internal
  count: Number? // OCN2a

class ObjectMapEntry // OME*, internal
