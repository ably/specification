...
* @(RTO17)@ The @RealtimeObjects@ instance must maintain an internal sync state to track the status of synchronising the local objects data with the Ably service. This state determines when @RealtimeObjects#getRoot@ can return (per "RTO1c":#RTO1c) and drives the emission of sync events (see "RTO18":#RTO18)
...
* @(RTO20)@ @RealtimeObjects#offAll@ function - deregisters all event listeners previously registered via @RealtimeObjects#on@ ("RTO18":#RTO18)
...
class RealtimeObjects: // RTO*
  ...
  on(ObjectsEvent event, (() ->) callback) -> OnObjectsEventResponse // RTO18
  off(() ->) // RTO19
  offAll() // RTO20
...
interface OnObjectsEventResponse: // RTO18f
  off() // RTO18f1
