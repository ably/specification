...
* @(CHA-RST6)@ @[Testable]@ All URL path variables (that is, the path components given in angle brackets, e.g. @<roomName>@) must be percent-encoded to produce a string that conforms to the @segment@ production as defined in "RFC 3986 Section 3.3":https://datatracker.ietf.org/doc/html/rfc3986#section-3.3. (Note that this differs from the @path@ production in that it requires that @/@ be percent-encoded.).
