...
* @(CHA-RST6)@ This specification requires that certain URL path segments be percent-encoded. When this is the case, this encoding procedure must produce a string that conforms to the @segment@ production as defined in "RFC 3986 Section 3.3":https://datatracker.ietf.org/doc/html/rfc3986#section-3.3. (Note that this differs from the @path@ production in that it requires that @/@ be percent-encoded.)
* @(CHA-RST4)@ @[Testable]@ Where a URL path contains a @roomName@ variable, that value shall be percent-encoded per @CHA-RST6@.
* @(CHA-RST5)@ @[Testable]@ Where a URL path contains a @timeserial@ variable, that value shall be percent-encoded per @CHA-RST6@.
